...
/// An example implementation of a simple cell with a frame around.
///
...
class FramedStoryCell: UICollectionViewCell {
    ...
    /// frame to show read status
    @IBOutlet weak var framerView: UIView!
    /// container for cell contents
    @IBOutlet weak var containerView: UIView!
    /// icon showing that stories have content with sound
    ...
    override func awakeFromNib() {
        ...
        /// cell frame edge rounding setting
...
extension FramedStoryCell: StoryCellProtocol {
    ...
    func setOpened(_ value: Bool) {
        /// If the story has already been read by the user,
        /// in this case we change the display of the frame
    ...
    func setHighlight(_ value: Bool) {
        /// If the user clicked on a cell but did not release it,
        /// we will highlight by resizing the cell to improve UX
        UIView.animate(withDuration: 0.2) {
            /// more details can be seen in ``UIKit/UIView/setTransform(withScale:anchorPoint:)``
            self.containerView.setTransform(withScale: value ? 0.95 : 1.0, anchorPoint: CGPoint(x: 0.5, y: 0.5))
        }
    }
